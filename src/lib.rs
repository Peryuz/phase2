...
/// circuit into a QAP.
struct KeypairAssembly<E: PrimeField> {
    num_inputs: usize,
    num_aux: usize,
    num_constraints: usize,
    at_inputs: Vec<Vec<(E, usize)>>,
    bt_inputs: Vec<Vec<(E, usize)>>,
    ct_inputs: Vec<Vec<(E, usize)>>,
    at_aux: Vec<Vec<(E, usize)>>,
    bt_aux: Vec<Vec<(E, usize)>>,
    ct_aux: Vec<Vec<(E, usize)>>,
}

impl<E: PrimeField> ConstraintSystem<E> for KeypairAssembly<E> {
    type Root = Self;
    ...
    {
        fn eval<E: PrimeField>(
            l: LinearCombination<E>,
            inputs: &mut [Vec<(E, usize)>],
            aux: &mut [Vec<(E, usize)>],
...
fn merge_pairs<G: PairingCurveAffine>(v1: &[G], v2: &[G]) -> (G, G) {
    ...
    let s = Arc::new(Mutex::new(G::identity()));
    let sx = Arc::new(Mutex::new(G::identity()));
    ...
    crossbeam::scope(|scope| {
        for (v1, v2) in v1.chunks(chunk).zip(v2.chunks(chunk)) {
            ...
            scope.spawn(move || {
                ...
                let mut wnaf = Wnaf::new();
                let mut local_s = G::identity();
                let mut local_sx = G::identity();
